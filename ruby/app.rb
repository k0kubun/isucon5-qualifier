...
SQL
    ...
    comments_of_friends = []
    comments_10 = db.query("SELECT * FROM comments WHERE user_id IN (#{friend_ids.join(',')}) ORDER BY id DESC LIMIT 10")
    eager_load_entry(comments_10).each do |comment|
      entry = comment[:entry]
